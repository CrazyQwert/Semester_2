...
/**
 * Einführung in die Softwareentwicklung 2 (ESE2), SS2016 Vorführaufgabe 5 - Weg
 * durch ein Labyrinth (rekursiv)
 * 
...
public class Maze_Skeleton {
	...
	public void init() {
		for (int i = 0; i < maze.length; i++) {
			for (int j = 0; j < maze[i].length; j++) {
				...
				else if (java.lang.Math.random() > 0.7) { // 0.7 = "Dichte" der
															// belegten Felder
															// (je kleiner,
															// desto voller...)
					maze[i][j] = BLOCKED_CHAR; // blockiert
	...
	public boolean reachable(int x, int y) {
		...
		if (maze[x][y] == END_CHAR) return true;
		
		//only check when you were not already there and it is not blocked
		if (maze[x][y] != BLOCKED_CHAR && maze[x][y] != WAY_CHAR && maze[x][y] != VISITED_CHAR) {
			maze[x][y] = WAY_CHAR;	//you now visited this place
